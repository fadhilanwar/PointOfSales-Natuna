                    ...
                    @auth
                        ...
                        <div class="flex items-center gap-3">
                            <div
                                ...
                                <span class="text-sm">Halo,
                                    {{ auth()->user()->shop_name ?? auth()->user()->username }}</span>
                            </div>
    ...
    @if (!request()->routeIs(['checkout.index', 'checkout.payment']))
        <nav
            ...
            <a href="{{ auth()->check() ? route('cart.index') : route('login') }}"
                ...
                @auth
                    <span
                        class="absolute top-1 right-3 h-2.5 w-2.5 rounded-full bg-[#06728A] border-2 border-white"></span>
                @endauth
            </a>
            <a href="{{ auth()->check() ? route('orders.index') : route('login') }}"
                class="flex flex-col items-center p-2 {{ request()->routeIs('orders.index') ? 'text-[#06728A]' : 'text-gray-400 hover:text-[#06728A]' }} cursor-pointer transition-all duration-300 ease-in-out hover:-translate-y-1">
                <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"

...
@section('content')
    <div class="h-[calc(100vh-80px)] flex flex-col -m-6 p-6 bg-slate-100">
        ...
        @if (session('success'))
            <div
                class="mb-4 p-4 text-sm font-medium text-emerald-800 rounded-lg bg-emerald-50 border border-emerald-200 shadow-sm flex justify-between items-center">
                <span class="font-bold">{{ session('success') }}</span>

                @if (session('order_id'))
                    <!-- Tombol Cetak Struk dengan target="_blank" -->
                    <a href="{{ route('admin.pos.receipt', session('order_id')) }}" target="_blank"
        ...
        <div class="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0">
            ...
            <div
                class="lg:col-span-8 flex flex-col h-full bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-4 border-b border-slate-100 bg-white">
                    <div class="relative w-full">
                        ...
                        <input type="text" id="search-product"
                            class="bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-lg focus:ring-[#0a7b8c] focus:border-[#0a7b8c] block w-full pl-10 p-3 outline-none"
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                        @foreach ($products as $product)
                            <div class="product-card bg-white rounded-xl border border-slate-200 hover:border-slate-300 p-3 flex flex-col shadow-sm transition-all"
                                ...
                                <div
                                    class="h-24 w-full bg-slate-100 rounded-lg mb-3 overflow-hidden flex items-center justify-center">
                                ...
                                <h3 class="font-bold text-slate-800 text-sm leading-tight mb-1 line-clamp-2"
                                    title="{{ $product->name }}">{{ $product->name }}</h3>
                                <div class="mt-auto mb-3 flex justify-between items-end">
                                    <span class="font-bold text-[#0a7b8c] text-sm">Rp
                                        {{ number_format($product->base_price, 0, ',', '.') }}</span>
                                    <span
                                        class="text-xs font-semibold px-2 py-0.5 rounded {{ $product->stock < $product->low_stock_threshold ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-500' }}">Stok:
                                        {{ $product->stock }}</span>
                                </div>
                                ...
                                <form action="{{ route('admin.pos.cart.add') }}" method="POST" class="m-0">
                                    ...
                                    @if ($product->stock > 0)
                                        <button type="submit"
                                            ...
                                            class="w-full bg-slate-50 hover:bg-[#0a7b8c] border border-slate-200 hover:border-[#0a7b8c] text-slate-600 hover:text-white font-bold py-2 rounded-lg text-sm transition-all shadow-sm cursor-pointer">
                                            Tambah
                                        </button>
                                    @else
                                        <button type="submit"
            ...
            <div class="lg:col-span-4 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col h-full">
                ...
                <div class="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-xl">
                    <h3 class="font-bold text-slate-800 flex items-center gap-2">
                        <svg class="w-5 h-5 text-[#0a7b8c]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <span
                        class="text-xs font-bold px-2.5 py-1 bg-white border border-slate-200 rounded-md text-slate-500">{{ $invoice_number }}</span>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-3 bg-slate-50/30">
                    @forelse($cartItems as $item)
                        <div
                            class="flex justify-between items-center p-3 mb-3 bg-white border border-slate-100 rounded-xl shadow-sm hover:border-[#0a7b8c]/30 transition-colors">

                            <div class="flex-1 min-w-0 pr-3">
                                <h4 class="font-bold text-sm text-slate-800 truncate" title="{{ $item->product->name }}">
                                    {{ $item->product->name }}</h4>
                                <div class="text-[#0a7b8c] font-bold text-xs mt-1">Rp
                                    {{ number_format($item->product->base_price, 0, ',', '.') }}</div>
                            </div>
                            ...
                            <div class="flex items-center gap-1.5">
                                <form action="{{ route('admin.pos.cart.update') }}" method="POST" class="m-0">
                                    ...
                                    <button type="submit"
                                        class="w-7 h-7 flex items-center justify-center bg-slate-100 border border-slate-200 rounded-md text-slate-600 font-bold hover:bg-slate-200 transition-colors">-</button>
                                </form>
                                ...
                                <form action="{{ route('admin.pos.cart.update') }}" method="POST" class="m-0 ml-1">
                                    ...
                                    <button
                                        onclick="return confirm('Yakin menghapus barang {{ $item->product->name }} dari Keranjang Belanja.')"
                                        type="submit"
                                        class="text-red-400 hover:text-red-600 p-1.5 bg-red-50 hover:bg-red-100 rounded-md transition-colors"
                ...
                <div
                    class="border-t border-slate-200 bg-white p-5 rounded-b-xl space-y-5 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-10 relative">
                    ...
                    <div class="flex justify-between items-end border-b border-slate-100 pb-4">
                        ...
                        <span class="text-3xl font-extrabold text-[#0a7b8c] leading-none">Rp
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="payment_method" id="payment-method" form="pos-form"
                                ...
                                <option value="cod">Cash (Tunai)</option>
                                <option value="transfer">Transfer Bank (Tunai)</option>
                    ...
                    <div class="p-3 border-b border-slate-100 bg-white">
                        ...
                        <select name="user_id" form="pos-form"
                            class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-lg focus:border-[#0a7b8c] p-2.5 outline-none font-medium transition-colors">
                            <option value="">-- Pilih Pelanggan (Wajib) --</option>
                            @foreach ($customers as $customer)
                                <option value="{{ $customer->id }}">{{ $customer->name }}
                                    {{ $customer->shop_name ? '(' . $customer->shop_name . ')' : '' }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <button type="submit" form="pos-form"
                        class="w-full bg-[#0a7b8c] cursor-pointer text-white font-bold py-3.5 rounded-xl shadow-lg shadow-cyan-500/30 hover:bg-[#075e6b] hover:shadow-cyan-500/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed uppercase tracking-wider text-sm flex items-center justify-center gap-2"

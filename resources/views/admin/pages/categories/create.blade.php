...
<div class="rounded-xl border border-slate-200 bg-white shadow-sm p-6 max-w-2xl">
    <form action="{{ route('admin.categories.store') }}" method="POST">
        ...
        <div class="mb-5">
            ...
            <input type="text" id="category_name" name="category_name" value="{{ old('category_name') }}" placeholder="Contoh: Sembako, Minuman..." 
                   class="w-full rounded-lg border bg-transparent py-2.5 px-4 outline-none focus:border-[#0a7b8c] focus:ring-1 focus:ring-[#0a7b8c] @error('category_name') border-red-500 @enderror">
            @error('category_name')
        ...
        <div class="mb-6">
            ...
            <textarea id="description" name="description" rows="4" placeholder="Tuliskan deskripsi kategori di sini..."
                      class="w-full rounded-lg border border-slate-300 bg-transparent py-2.5 px-4 outline-none focus:border-[#0a7b8c] focus:ring-1 focus:ring-[#0a7b8c]">{{ old('description') }}</textarea>
        </div>

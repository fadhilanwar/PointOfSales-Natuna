...
<div class="p-6 bg-slate-50 min-h-[calc(100vh-80px)] -m-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
            ...
            <div><p class="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Uang Keluar</p><h3 class="text-2xl font-black text-slate-800">Rp {{ number_format($totalSpending, 0, ',', '.') }}</h3></div>
        </div>
        <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6 print:hidden">
        <form method="GET" action="{{ route('report.supply') }}" class="flex flex-wrap gap-4 items-end">
            <div>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('report.supply') }}" class="bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold py-2.5 px-5 rounded-lg text-sm transition-colors">Reset</a>
            </div>
        </form>
    </div>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-6 py-4 text-center print:hidden">
                        <a href="{{ route('report.supply.show', $s->id) }}" class="inline-flex items-center gap-1.5 rounded-lg bg-blue-500 border border-slate-200 py-1.5 px-3 text-xs font-semibold text-white hover:bg-blue-400 transition-all shadow-sm">Detail Suplai</a>
                    </td>
                </tr>
                @empty
                <tr><td colspan="5" class="p-8 text-center text-slate-500">Tidak ada data pembelian/suplai pada periode tanggal ini.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>

...
@section('content')
    <div class="mb-6 flex items-center justify-between">
        <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Tambah Produk Baru</h2>
    ...
    <div class="rounded-xl border border-slate-200 bg-white shadow-sm p-6 max-w-4xl">
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="Minyak Goreng 2 Liter"
                            class="w-full rounded-lg border border-slate-300 py-2.5 px-4 outline-none focus:border-[#0a7b8c] focus:ring-1 focus:ring-[#0a7b8c] font-medium"
                            required>
                        @error('name')
                            <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p>
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="mb-2 block text-sm font-semibold text-slate-800">Stok Awal <span
                                    class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-800">Kategori
                        ...
                        <select name="category_id"
                            class="w-full rounded-lg border border-slate-300 py-2.5 px-4 outline-none focus:border-[#0a7b8c] font-medium">
            ...
            <div class="mt-3 p-5 font-semibold text-xs">
                <h2>Preview</h2>
            ...
            <div class="flex justify-end border-t pt-6">
                <button type="submit"
                    class="rounded-lg cursor-pointer bg-[#0a7b8c] py-2.5 px-8 font-semibold text-white hover:bg-[#075e6b] transition-all shadow-sm">Simpan
                    Produk</button>
            </div>
        </form>
    </div>
